...
negative strands. The scores for G4s on the positive strand are positive, 
while those on the negative strand are negative, resulting in a bimodal 
distribution of `max_score` and `score`. Please note that the positive or 
negative score reflects only the strand type of the G4, whereas the capability 
of the G4 structure to form is determined by the absolute value of the score.
...
If `raw_score_threshold` (a positive number, but no more that 4) is specified, 
`filterSNVImpact` will filter out records where the absolute value of the 
original G4Hunter score is below this threshold.

If `mut_score_threshold` (a positive number, but no more that 4) is specified, 
