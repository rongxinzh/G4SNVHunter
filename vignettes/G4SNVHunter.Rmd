...
Single nucleotide variants (SNVs) are a common form of genetic variation. 
When occurring within G4 regions, they can destabilize G4 structures and, 
in turn, disrupt their regulatory functions.

`G4SNVHunter` is designed to evaluate the potential impact of SNVs on G4 
stability, but it can also be used to assess single nucleotide polymorphisms 
...
`G4SNVHunter` allows users to quickly determine which G4 structures might be 
affected by SNVs (or SNPs) using only reference sequence data (`DNAStringSet` 
object) and the substitution information of the variants (`GRanges` object). 
This package is particularly beneficial for researchers focusing on specific 
SNVs, as it facilitates the assessment of the potential impact of these 
variants on G4 structures and supports the selection of experimental targets 
for subsequent causal analysis.

By integrating complex functionalities into a straightforward function, 
`G4SNVHunter` simplifies the analysis process, making it accessible even to 
those with minimal prior experience in G4 research or basic programming skills.
...
whether these SNVs can affect their stability.
...
Given that some SNVs may have minimal effects on G4 stability, we need to 
...
We can observe that the mutated G4s generally show a trend of decreased 
stability.
